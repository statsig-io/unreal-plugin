...
void QueryStatsig() {
  ...
  Log(FString::Format(
          TEXT("an_experiment.a_string -- {0} -- {1} -- {2}"),
          {
            value.IsEmpty() ? "NULL" : value,
            experiment.GetEvaluationDetails().reason,
            experiment.GetGroupName().value_or("NULL")
          }),
      FColor::Green);
  ...
  Log(FString::Format(
          TEXT("not_an_experiment.not_real -- {0} -- {1} -- {2}"),
          {
              not_a_value.IsEmpty() ? "NULL" : not_a_value,
              not_an_experiment.GetEvaluationDetails().reason,
              not_an_experiment.GetGroupName().value_or("NULL"),
          }),
      FColor::Green);
  ...
  Log(FString::Format(TEXT("a_layer.a_string -- {0} -- {1} -- {2}"),
                      {
                        param.has_value() ? param.value()->AsString() : TEXT(""),
                       layer.GetEvaluationDetails().reason,
                        layer.GetGroupName().value_or("NULL"),
                      }),
      FColor::Green);
